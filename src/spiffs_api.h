...
protected:
    ...
    bool _tryMount() {
        ...
        spiffs_hal_init();
        auto err = SPIFFS_mount(&_fs, &config, _workBuf.get(), _fdsBuf.get(), fdsBufSize, _cacheBuf.get(), cacheBufSize, &SPIFFSImpl::_check_cb);
...
public:
    SPIFFSFileImpl(SPIFFSImpl *fs, spiffs_file fd) : _fs(fs), _fd(fd), _written(false) {
...
public:
    SPIFFSDirImpl(const String &pattern, SPIFFSImpl *fs, spiffs_DIR &dir) : _pattern(pattern), _fs(fs), _dir(dir), _dirHead(dir), _valid(false) {
    }
    ...
    FileImplPtr openFile(OpenMode openMode, AccessMode accessMode) override {
        ...
        if (fd < 0) {
            DEBUGV("SPIFFSDirImpl::openFile: fd=%d path=`%s` openMode=%d "
                   "accessMode=%d err=%d\r\n",
